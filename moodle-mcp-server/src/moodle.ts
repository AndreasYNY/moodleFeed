...
function encodeParams(params: Record<string, unknown>, prefix?: string, body = new URLSearchParams()): URLSearchParams {
...
async function parseMoodleResponse<T>(response: Response): Promise<T> {
  ...
  if (data.exception || data.errorcode) {
    throw new MoodleApiError(
      (data.message as string) || 'Moodle API error',
...
export const Moodle = {
  ...
  assignments: (baseUrl: string, token: string, courseIds: number[]) =>
    moodleCall<{ courses: Array<{ id: number; fullname: string; assignments: unknown[] }> }>(
      baseUrl,
      token,
